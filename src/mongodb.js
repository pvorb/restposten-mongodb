...
Mongodb.prototype.save = function(/* id, doc, callback */) {
  var self = this, args = Array.prototype.slice.call(arguments, 0), callback = args
      .pop(), doc = args.pop(), id = args.pop();

  if (!id && !doc.id) {
    // no id available, so we must be inserting a new document. same as `post`
    // :D
...
Mongodb.prototype.destroy = function(id, callback) {
  ...
  return this.request(function(err) {
    ...
    self.client.remove({
      ...
      callback(null, self.cache.get(id) || {
        id : id
      });
    });
  });
